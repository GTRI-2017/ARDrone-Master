/*
GTRI ATAS
All2.0 -- Combined nodes
Created 6/27/17
Updated 7/7/17 :: 8:56 PM
*/
...
//this is called when a new message arrives on the nav topic.
  ...
  vx_=msg_in.vx*0.001;
	...
	vz_=msg_in.vz*0.001;
  newa = msg_in.altd;
  tc = msg_in.tags_count;  //tc needs to depend on both forward and bottom camera
	}
...
void corner(int argc, char **argv)
	{
...
void octagonthing(int argc, char **argv)
	{
  ros::init(argc, argv,"ardrone_movement"); //NOTE: "ardrone_search" is name of node
 	ros::NodeHandle node
	ros::Rate loop_rate(50);
...
////////////////////////////////////////////////////////////////
    ...
    while (hyp > .1)
      ...
      //double vz = (1/t)*dcomponents[2]; //version for distance divided by two
      ...
      while ((double)ros::Time::now().toSec() < begint+0.5 && tc > 0) // half second run time, can be changed for run legth
        {
        //runs the velocities
      ...
      if (tc = 0)
        {
        explore(3,b);
        octagonthing(5,a); // when we get avoidance change it to roomba
        }
      }
...
////////////////////////////////////////////////////////////////
  ...
  pub_land.publish(land_msg); //lands the drone if ROS isn't ok
  ros::spinOnce();
  loop_rate.sleep();
	}
